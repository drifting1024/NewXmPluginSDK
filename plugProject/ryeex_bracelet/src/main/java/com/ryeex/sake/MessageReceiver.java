...
import static com.ryeex.sake.MainActivity.TAG;
...
public class MessageReceiver implements IXmPluginMessageReceiver {
    ...
    public boolean handleMessage(Context context, XmPluginPackage xmPluginPackage, int type,
        ...
        Logger.d(TAG, "MessageReceiver.handleMessage plugin-version:" + xmPluginPackage.packageVersion + " type:" + type);
        switch (type) {
            case LAUNCHER: {// 启动入口
                XmPluginHostApi.instance().startActivity(context, xmPluginPackage, intent, deviceStat.did, MainActivity.class);
                return true;
            }
            case PUSH_MESSAGE: {
                // 订阅消息push通知
